...
func (p *Provider) doTunnelHTTPRequest(method string, path string, routeKind string, modelName string, body []byte, contentType string, isStream bool) (*http.Response, *types.OpenAIErrorWithStatusCode) {
	...
	if gateway.Global == nil {
		return nil, &types.OpenAIErrorWithStatusCode{
			OpenAIError: types.OpenAIError{
				Message: "tokilake gateway not initialized",
				Type:    "one_api_error",
			},
	...
	resp, requestID, requestErr := gateway.Global.DoTunnelRequest(p.requestContext(), p.Channel.Id, tunnelReq)
	if requestErr != nil {
