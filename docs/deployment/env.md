...
20. `HASHIDS_SALT` ：Sqids 字母表，用于混淆用户令牌信息，可空。为空时使用默认字母表 `abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789`。如设置，则必须保证字母表中无重复字符；不要把随机十六进制字符串直接填到这里。
