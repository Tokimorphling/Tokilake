...
export default function MultiUserStats() {
  ...
  const handleSearch = async () => {
    if (!usernames.trim()) {
      showError(t('multi_user_stats.messages.enter_usernames'));
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        if (data.length === 0) {
          showSuccess(t('multi_user_stats.messages.no_data'));
        } else {
          showSuccess(t('multi_user_stats.messages.data_found', { count: data.length }));
        }
      } else {
        showError(message);
      }
      ...
      console.error(error);
      showError(t('multi_user_stats.messages.query_failed') + (error.response?.data?.message || error.message));
    }
    setSearching(false);
  };
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      showSuccess(t('multi_user_stats.messages.export_success'));
    } catch (error) {
      console.error(error);
      showError(t('multi_user_stats.messages.export_failed') + (error.response?.data?.message || error.message));
    }
    setSearching(false);
  };
  ...
  const quotaChartData = {
    options: {
      ...
      yaxis: {
        title: { text: t('multi_user_stats.charts.quota_consumption'), style: { color: theme.palette.text.primary } },
        labels: {
    ...
    series: [
      {
        name: t('multi_user_stats.charts.quota_consumption'),
  ...
  const modelUsageChartData = {
    options: {
        ...
        pie: {
          donut: {
            ...
            labels: {
              ...
              total: {
                show: true,
                label: t('multi_user_stats.charts.total_calls'),
      ...
      tooltip: {
        theme: theme.palette.mode,
        y: { formatter: (val) => val.toLocaleString() + ' ' + t('multi_user_stats.charts.unit_times') }
      }
    },
    ...
    <>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Stack direction="column" spacing={1}>
          <Typography variant="h2">{t('multi_user_stats.title')}</Typography>
          <Typography variant="subtitle1" color="text.secondary">
            {t('multi_user_stats.description')}
          </Typography>
          <Typography variant="body2" color="text.secondary">
            {t('multi_user_stats.subtitle')}
          </Typography>
        </Stack>
      </Stack>
      ...
      <Card sx={{ mb: 3, p: 3 }}>
        <Stack spacing={3}>
          <TextField
            fullWidth
            label={t('multi_user_stats.usernames_label')}
            placeholder={t('multi_user_stats.usernames_placeholder')}
            value={usernames}
            onChange={(e) => setUsernames(e.target.value)}
            helperText={t('multi_user_stats.usernames_helper')}
          />

          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <Stack direction="row" spacing={2}>
              <DatePicker
                label={t('multi_user_stats.start_date')}
                value={startDate}
              ...
              />
              <DatePicker
                label={t('multi_user_stats.end_date')}
                value={endDate}
          ...
          <Stack direction="row" spacing={2}>
            ...
            >
              {t('multi_user_stats.query_button')}
            </Button>
            <Button
              variant="outlined"
            ...
            >
              {t('multi_user_stats.export_button')}
            </Button>
          </Stack>
        </Stack>
      </Card>
        ...
        <>
              ...
              <Grid item xs={12} sm={6} md={4} key={index}>
                ...
                >
                  <Stack spacing={2}>
                    ...
                    <Box>
                      <Typography variant="body2" color="text.secondary">
                        {t('multi_user_stats.cards.total_requests')}
                      </Typography>
                    ...
                    <Box>
                      <Typography variant="body2" color="text.secondary">
                        {t('multi_user_stats.cards.quota_consumption')}
                      </Typography>
                    ...
                    <Grid container spacing={1}>
                      <Grid item xs={6}>
                        <Typography variant="caption" color="text.secondary">
                          {t('multi_user_stats.cards.prompt_tokens')}
                        </Typography>
                      ...
                      <Grid item xs={6}>
                        <Typography variant="caption" color="text.secondary">
                          {t('multi_user_stats.cards.completion_tokens')}
                        </Typography>
                        <Typography variant="body2" sx={{ fontWeight: 500 }}>
                    ...
                    <Box>
                      <Typography variant="caption" color="text.secondary">
                        {t('multi_user_stats.cards.request_time')}
                      </Typography>
                      <Typography variant="body2" sx={{ fontWeight: 500 }}>
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} lg={6}>
              <MainCard sx={{ borderRadius: '16px' }}>
                <Typography variant="h3" sx={{ mb: 2, fontWeight: 600 }}>
                  {t('multi_user_stats.charts.quota_comparison')}
                </Typography>
            ...
            <Grid item xs={12} lg={6}>
              <MainCard sx={{ borderRadius: '16px' }}>
                <Typography variant="h3" sx={{ mb: 2, fontWeight: 600 }}>
                  {t('multi_user_stats.charts.model_distribution')}
                </Typography>
                {modelUsage.length > 0 ? (
                  <Paper elevation={0} sx={{ bgcolor: 'transparent', p: 2 }}>
                  ...
                  >
                    <Typography variant="h4" color="text.secondary">
                      {t('multi_user_stats.charts.no_data')}
                    </Typography>
                  </Box>
                )}
              </MainCard>
      ...
      {!searching && statistics.length === 0 && (
        <Card sx={{ p: 6, textAlign: 'center' }}>
          ...
          <Typography variant="h4" color="text.secondary" sx={{ mt: 2 }}>
            {t('multi_user_stats.messages.search_hint')}
          </Typography>
        </Card>
      )}
    </>
